...
import 'package:playground/generated/l10n.dart';
...
abstract class DialogsHelpers {
  ...
  static Future showMessageDialog(material.BuildContext context,
      ...
      showDialog(context,
          message: message, confirmationCallback: confirmationCallback);
  ...
  static Future showMessageOverlay(
      material.BuildContext context, String message,
      {int duration = 3}) {
    _dismissFauces(context);
    final completer = _createCompleter(duration);
    FlashHelper.blockMessage(context,
        message: message, dismissCompleter: completer);
  ...
  static Future<void> showSuccessOverlay(
    ...
    _dismissFauces(context);
    final completer = _createCompleter(duration);
    FlashHelper.blockSuccessMessage(context,
        message: message, dismissCompleter: completer);
    return completer.future;
  }
  ...
  static Future showErrorOverlay(material.BuildContext context, String message,
    ...
    final completer = _createCompleter(duration);
    Future.delayed(duration.seconds).then((_) => completer.complete());
    FlashHelper.blockErrorMessage(context,
        message: message, dismissCompleter: completer);
    return completer.future;
  }
